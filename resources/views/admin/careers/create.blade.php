    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.careers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('job_category') ? 'is-invalid' : '' }}" name="job_category" id="job_category">
                    ...
                    @foreach(App\Models\Career::JOB_CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('job_category', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('salary_range') ? 'is-invalid' : '' }}" type="number" name="salary_range" id="salary_range" value="{{ old('salary_range', '0') }}" step="0.01">
