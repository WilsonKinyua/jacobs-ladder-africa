...
class TeamController extends Controller
{
    ...
    public function store(StoreTeamRequest $request)
    {
        ...
        if ($request->input('profile_picture', false)) {
            $team->addMedia(storage_path('tmp/uploads/' . basename($request->input('profile_picture'))))->toMediaCollection('profile_picture');
        }
    ...
    public function update(UpdateTeamRequest $request, Team $team)
    {
        ...
        if ($request->input('profile_picture', false)) {
            if (!$team->profile_picture || $request->input('profile_picture') !== $team->profile_picture->file_name) {
                if ($team->profile_picture) {
                    $team->profile_picture->delete();
                }
                $team->addMedia(storage_path('tmp/uploads/' . basename($request->input('profile_picture'))))->toMediaCollection('profile_picture');
            }
        } elseif ($team->profile_picture) {
            $team->profile_picture->delete();
        }
